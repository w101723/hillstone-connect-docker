...
- 配置和应用数据使用相对路径持久化
- GitHub Actions 自动构建并推送多架构镜像到 GHCR
...
```bash
docker pull ghcr.io/w101723/hillstone-connect:latest
```

默认的 `.env.example` 和 `compose.yaml` 均使用：

```text
ghcr.io/w101723/hillstone-connect:latest
```
...
docker buildx build \
  ...
  -t ghcr.io/w101723/hillstone-connect:latest \
  --push .
```
...
```text
ghcr.io/w101723/hillstone-connect
```
...
```text
ghcr.io/w101723/hillstone-connect:latest
ghcr.io/w101723/hillstone-connect:main
ghcr.io/w101723/hillstone-connect:sha-<commit>
```
...
```bash
docker pull ghcr.io/w101723/hillstone-connect:v1.0.0
```
